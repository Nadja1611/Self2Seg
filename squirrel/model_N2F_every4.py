# -*- coding: utf-8 -*-
"""
Created on Wed Jun 21 14:19:43 2023
...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun  7 13:10:41 2023
...
class TwoCon(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.conv1 = nn.Conv2d(in_channels, out_channels, 5, padding=2, padding_mode="reflect")
...
##################### accelerated segmentation algorithm bg constant############################
    def segmentation_step2denoisers_acc_bg_constant(self,f, iterations, gt):
        ...
        if self.method == "joint":
            print("filter")
            diff1 = torch.nn.functional.conv2d(diff1, kernel,padding = 1)
            self.diffi1=diff1
        diff2 = ((torch.clone(f_orig - self.mu_r2))).float()
        if self.method == "joint":
            print("filter")
            diff2 = torch.nn.functional.conv2d(diff2, kernel,padding = 1)
            self.diffi2=diff2
       # diff1 = torch.nn.functional.conv2d(diff2, kernel,padding = 1)
        plt.imshow((diff2**2).cpu()[0], cmap="winter")
        plt.title("diff")
        plt.show()
        plt.imshow((diff1**2).cpu()[0], cmap="winter")
        plt.title("diff1")
        plt.show()
        plt.imshow(((diff1-diff2).cpu()[0]>0),cmap="winter" )
        plt.show()
    ...
    def denoising_step_r1(self):
        f = torch.clone(self.f)
        self.f1 = torch.sum(f*(self.x))/torch.sum(self.x)

    def denoising_step_r2(self):
        f = torch.clone(self.f)
        self.mu_r2 = torch.sum(f*(1-self.x))/torch.sum(1-self.x)
        
    def denoising_step_r3(self, maske):
        f = torch.clone(self.f)
        self.mu_r2 = torch.sum(f*(1-maske))/torch.sum(1-maske)
           ...
           # if  self.first_loss > self.current_loss:
            while timesince <= 100:
                ...
                if region == "foreground":
                    ...
                    loss1 = torch.sum(torch.nn.functional.conv2d((outputs-labello), kernel, padding=1)**2*loss_mask)#+ torch.sum(torch.min(self.f1)-torch.clip(outputs,max=torch.min(self.f1)))#+ 0.1*torch.sum((outputs -  torch.mean(outputs))**2)#/torch.sum(loss_mask)
                    loss1.backward()
                    self.optimizer.step()
                ...
                with torch.no_grad():
                    ...
                    if region == "foreground":
                        ...
                        outputstest = self.net1(img_test.detach()).detach()
    ...
    def N2Fstep2(self, mask, region="foreground"):
        ...
        while self.notdone: 
            ...
            imgin = np.zeros((Zshape[0]//4,Zshape[1]),dtype=np.float32)
            imgin2 = np.zeros((Zshape[0]//4,Zshape[1]),dtype=np.float32)
                     
            imgin_mask = np.zeros((Zshape[0]//4,Zshape[1]),dtype=np.float32)
            imgin2_mask = np.zeros((Zshape[0]//4,Zshape[1]),dtype=np.float32)
            for i in range(imgin.shape[0]):
                for j in range(imgin.shape[1]):
                    if j % 2 == 0:
                        imgin[i,j] = imgZ[4*i+1,j]
                        imgin2[i,j] = imgZ[4*i,j]
                        imgin_mask[i,j] = imgM[4*i+1,j]
                        imgin2_mask[i,j] = imgM[4*i,j]
                    if j % 2 == 1:
                        imgin[i,j] = imgZ[4*i,j]
                        imgin2[i,j] = imgZ[4*i+1,j]
                        imgin_mask[i,j] = imgM[4*i,j]
                        imgin2_mask[i,j] = imgM[4*i+1,j]
            imgin = torch.from_numpy(imgin)
            imgin = torch.unsqueeze(imgin,0)
            imgin = torch.unsqueeze(imgin,0)
            imgin = imgin.to(self.device)
            ...
            imgin3 = np.zeros((Zshape[0],Zshape[1]//4),dtype=np.float32)
            imgin4 = np.zeros((Zshape[0],Zshape[1]//4),dtype=np.float32)
            imgin3_mask = np.zeros((Zshape[0],Zshape[1]//4),dtype=np.float32)
            imgin4_mask = np.zeros((Zshape[0],Zshape[1]//4),dtype=np.float32)
            for i in range(imgin3.shape[0]):
                for j in range(imgin3.shape[1]):
                    if i % 2 == 0:
                        imgin3[i,j] = imgZ[i,2*j+1]
            ...
            listimgV1 = [[listimgV[0],listimgV[1]]]
            listimgV1_rot = [[np.rot90(listimgV[0]),np.rot90(listimgV[1])]]

            listimgV2 = [[listimgV[1],listimgV[0]]]
            listimgV2_rot = [[np.rot90(listimgV[1]),np.rot90(listimgV[0])]]

            
            listimgH1 = [[listimgH[1],listimgH[0]]]
            listimgH1_rot = [[np.rot90(listimgH[1]),np.rot90(listimgH[0])]]

            listimgH2 = [[listimgH[0],listimgH[1]]]
            listimgH2_rot = [[np.rot90(listimgH[0]),np.rot90(listimgH[1])]]

            listimg = listimgH1+ listimgH1_rot + listimgH2+ listimgH2_rot + listimgV1+listimgV2 + listimgV1_rot + listimgV2_rot # + self.rotation_list
             
            listimgV1_mask = [[listimgV_mask[0],listimgV_mask[1]]]
            listimgV1_mask_rot = [[np.rot90(listimgV_mask[0]),np.rot90(listimgV_mask[1])]]

            listimgV2_mask = [[listimgV_mask[1],listimgV_mask[0]]]
            listimgV2_mask_rot = [[np.rot90(listimgV_mask[1]),np.rot90(listimgV_mask[0])]]

            listimgH1_mask = [[listimgH_mask[1],listimgH_mask[0]]]
            listimgH1_mask_rot = [[np.rot90(listimgH_mask[1]),np.rot90(listimgH_mask[0])]]

            listimgH2_mask = [[listimgH_mask[0],listimgH_mask[1]]]
            listimgH2_mask_rot = [[np.rot90(listimgH_mask[0]),np.rot90(listimgH_mask[1])]]

            listimg_mask = listimgH1_mask + listimgH1_mask_rot +listimgH2_mask+listimgH2_mask_rot + listimgV1_mask+listimgV2_mask + listimgV1_mask_rot + listimgV2_mask_rot#+ self.rotation_list_mask
