# -*- coding: utf-8 -*-
"""
Created on Sun May 28 09:17:01 2023
...
class TwoCon(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.conv1 = nn.Conv2d(in_channels, out_channels, 15, padding=7, padding_mode="reflect")
...
class Net(nn.Module):
    def __init__(self):
        super().__init__()
      #  self.conv6 = nn.Conv2d(1,16,15,padding=7)

        self.conv1 = TwoCon(1, 1)
           ...
           # if  self.first_loss > self.current_loss:
            while timesince <= 100:
                ...
                if region == "foreground":
                    ...
                    loss1 = torch.sum((outputs[:,:,1:-1,1:-1]-labello)**2*loss_mask)#+ torch.sum(torch.min(self.f1)-torch.clip(outputs,max=torch.min(self.f1)))#+ 0.1*torch.sum((outputs -  torch.mean(outputs))**2)#/torch.sum(loss_mask)
                    loss1.backward()
                    self.optimizer.step()
                ...
                with torch.no_grad():
                    ...
                    if region == "foreground":
                        ...
                        outputstest = outputstest[:,:,1:-1,1:-1]
                    elif region == "background":
                        inputs = torch.nn.functional.pad(img_test.detach(),(1,1,1,1),mode = 'reflect')
